...
delete from exposures; delete from edges; delete from resource_nodes;
delete from nodes; delete from resources; delete from clusters;

insert into clusters (id, user_id, title, colour) values
...
-- title, cluster, ability, days since exposure (null = unsown)
insert into nodes (user_id, title, slug, cluster_id, ability, ability_confidence, last_exposure_at, created_by, summary) values
-- Front-end: the hot cluster
...
update nodes set state = 'pending' where slug = 'server-components';

select count(*) as nodes, count(*) filter (where state='pending') as pending from nodes;
...
select 'a0000003-0000-0000-0000-000000000003', id, 0.9 from nodes where slug='database-indexing'
union all select 'a0000004-0000-0000-0000-000000000004', id, 0.9 from nodes where slug='milton-friedman'
union all select 'a0000005-0000-0000-0000-000000000005', id, 0.8 from nodes where slug='css-layout'
union all select 'a0000006-0000-0000-0000-000000000006', id, 0.8 from nodes where slug='industrial-revolution'
union all select 'a0000007-0000-0000-0000-000000000007', id, 0.9 from nodes where slug='rag'
union all select 'a0000007-0000-0000-0000-000000000007', id, 0.5 from nodes where slug='embeddings'
union all select 'a0000001-0000-0000-0000-000000000001', id, 0.9 from nodes where slug='react-hooks'
union all select 'a0000002-0000-0000-0000-000000000002', id, 0.9 from nodes where slug='supabase';

-- Edges so the graph has structure
insert into edges (user_id, from_node, to_node, kind, weight, created_by)
select '11111111-1111-1111-1111-111111111111', a.id, b.id, 'prereq', 0.9, 'ai'
from nodes a, nodes b where a.slug='javascript' and b.slug='react'
union all select '11111111-1111-1111-1111-111111111111', a.id, b.id, 'specialises', 0.9, 'ai'
from nodes a, nodes b where a.slug='react' and b.slug='react-hooks'
union all select '11111111-1111-1111-1111-111111111111', a.id, b.id, 'alternative', 0.7, 'ai'
from nodes a, nodes b where a.slug='javascript' and b.slug='typescript'
union all select '11111111-1111-1111-1111-111111111111', a.id, b.id, 'specialises', 0.8, 'ai'
from nodes a, nodes b where a.slug='react' and b.slug='react-native'
union all select '11111111-1111-1111-1111-111111111111', a.id, b.id, 'related', 0.8, 'ai'
from nodes a, nodes b where a.slug='postgresql' and b.slug='supabase'
union all select '11111111-1111-1111-1111-111111111111', a.id, b.id, 'related', 0.7, 'ai'
from nodes a, nodes b where a.slug='embeddings' and b.slug='vector-search'
union all select '11111111-1111-1111-1111-111111111111', a.id, b.id, 'prereq', 0.8, 'ai'
from nodes a, nodes b where a.slug='vector-search' and b.slug='rag'
union all select '11111111-1111-1111-1111-111111111111', a.id, b.id, 'alternative', 0.8, 'ai'
from nodes a, nodes b where a.slug='keynesianism' and b.slug='milton-friedman';
...
returns void language sql as $$
  ...
  select '11111111-1111-1111-1111-111111111111',
         (select id from nodes where slug = s1),
         (select id from nodes where slug = s2),
         k, w, 'ai'
  where exists (select 1 from nodes where slug = s1)
    and exists (select 1 from nodes where slug = s2)
