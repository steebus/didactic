...
| POST | `/api/clozes/[id]/review` | rating | clozes | Answer a cloze. `rating` is 1–4 on FSRS's own scale (again, hard, good, easy), not a word of this app's: the Tend sheet offers three of the four, and a client offering the fourth is scheduling on the same ruler rather than a variant. Answers `{ cloze, intervalDays, retrievability, wait }`, where `wait` is the next interval in words. Moves the row and appends to `cloze_reviews`, in that order — a missing log line costs a future refitting, a missing schedule move costs the reader the same card in a minute. Drops only `clozes`: answering a cloze is not an exposure and does not touch ability. |
