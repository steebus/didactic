...
| POST | `/api/resources/uploaded` | path, filename, consumed? | resources, topics | The bytes landed; file them and queue the read. The path is not taken on trust: it must be one of ours, carry the caller's own id, and have an object actually at it (400, then 404). |
