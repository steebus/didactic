...
| POST | `/api/resources/uploaded` | path, filename, consumed? | resources, topics | The bytes landed; file them, read the document's structure, and queue the passages. The path is not taken on trust: it must be one of ours, carry the caller's own id, and have an object actually at it (400, then 404); the size is re-checked against the object rather than against what the client claimed. Answers `outline: { chapters, source, pageCount } \| null` — read here rather than on the queue because the queue is a once-a-minute cron and the reader is choosing a fidelity seconds later, so the answer always arrived after the question. `chapters: 0` is ordinary and not a failure: an article has none. |
