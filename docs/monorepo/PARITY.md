...
| Write a lesson body; write it again | built | planned (5) | `api/lessons.writeBody`, `core/jobs` | Written on first open, and askable from the topic sheet for any lesson standing at **Not written** — two presses, because it is a minute of compute that cannot be taken back. The reader need not watch it: it goes on the bench, which reports from any sheet and offers the way in when it lands. The tab has to stay open, because the body is saved in one piece at the end. |
