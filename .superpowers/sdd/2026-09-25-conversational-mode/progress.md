...
Tasks 3,7,8,9: complete (commits d91e1b7, turbo lint/typecheck/test 12/12, build 45 pages). Ruling: useAskContext returns a reader function rather than a live value — lint forbids the setState-in-effect the plan implied, and the context is only wanted at press time. Ruling: AskButton wrapped in Suspense; a client hook reading the address in the root layout fails prerender with CLIENT_HOOK_DYNAMIC. Ruling: three sub-routes use a named dropCache() per the existing guard test.
Task 9 (parity): complete — one finding, four routes missing api-contract rows, fixed in f857935.
